import { PrismaClient, UserRole, CampaignType, CampaignStatus, EntryType, PrizeCategory, WinnerStatus } from '@prisma/client';
...
async function main() {
  ...
  const smartSavingsCampaign = await prisma.campaign.create({
    data: {
      ...
      type: CampaignType.DEPOSIT_BASED,
  ...
  const mobileBankingCampaign = await prisma.campaign.create({
    data: {
      ...
      type: CampaignType.TRANSACTION_BASED,
      status: CampaignStatus.ACTIVE,
    ...
    data: [
      {
        ...
        category: PrizeCategory.VEHICLE,
  ...
  for (const customerId of sampleCustomers) {
    ...
    for (let i = 0; i < numEntries; i++) {
      ...
      entries.push({
        customerId,
        campaignId: smartSavingsCampaign.id,
        entryType: i === 0 ? EntryType.ACCOUNT_OPENED : EntryType.DEPOSIT,
        entriesEarned,
