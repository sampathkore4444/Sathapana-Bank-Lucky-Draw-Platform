...
const customerToken = (customerId: string) =>
  jwt.sign({ role: 'CUSTOMER', customerId }, config.jwtSecret, { expiresIn: '15m' });

const staffToken = jwt.sign(
  { userId: 'user-1', email: 'admin@test.com', role: 'SUPER_ADMIN' },
  config.jwtSecret,
  { expiresIn: '1h' }
);
...
describe('Customer Module', () => {
  ...
  describe('POST /customer/login', () => {
    it('should issue a short-lived customer token', async () => {
  ...
  describe('GET /customer/dashboard', () => {
    it('should return customer dashboard summary from the token identity', async () => {
      ...
      const response = await request(app)
        ...
        .set('Authorization', `Bearer ${customerToken('CUST-000001')}`);
    ...
    it('should ignore a spoofed X-Customer-Id header for customer tokens', async () => {
      const prisma = require('../src/config/database').default;
      prisma.campaign.findMany.mockResolvedValue([
        {
          id: 'c1',
    ...
    it('should allow staff lookup via X-Customer-Id with a staff token', async () => {
      const prisma = require('../src/config/database').default;
      prisma.campaign.findMany.mockResolvedValue([
        {
          id: 'c1',
  ...
  describe('PUT /customer/notifications/:id/read', () => {
    ...
    it('should return 404 if the notification does not belong to the customer', async () => {
      ...
      const response = await request(app)
        .put('/customer/notifications/3f4bd0a2-0000-0000-0000-000000000000/read')
        .set('Authorization', `Bearer ${customerToken('CUST-999999')}`);

      expect(response.status).toBe(404);
    });
  });

  describe('GET /customer/campaigns/:campaignId/eligibility/:customerId', () => {
    it('should forbid a customer checking another customer eligibility', async () => {
      const response = await request(app)
        .get('/customer/campaigns/3f4bd0a2-0000-0000-0000-000000000000/eligibility/CUST-999999')
        .set('Authorization', `Bearer ${customerToken('CUST-000001')}`);
